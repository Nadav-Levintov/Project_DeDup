...
avltree * avltree_create(avltree_cmpfn compare, PMemory_pool pool)
{
   // avltree * tree = malloc(sizeof(avltree));
	...
	avltree *tree = NULL;
	ret_val = memory_pool_alloc(pool, sizeof(avltree), (uint32**)&tree);
...
{
    //free(node);
	//do nothing (will be free when memory poll will be deleted)
}
...
static void avltree_for_each_block_recursive(const avltreenode * root, void* data_set, uint32 curr_file_sn, avltree_for_block fun)
{
	if (root->left != NULL) {
		avltree_for_each_block_recursive(root->left, data_set, curr_file_sn, fun);
	}
	fun(root->data, data_set, curr_file_sn);
	if (root->right != NULL) {
		avltree_for_each_block_recursive(root->right, data_set, curr_file_sn, fun);
	}
}
...
{
	if (tree->root) {
		avltree_for_each_block_recursive(tree->root, data_set, curr_file_sn, fun);
	}
}
static void avltree_for_each_recursive_print(const avltreenode * root, FILE * pFile, avltree_for_print_func fun)
{
	if (root->left != NULL) {
		avltree_for_each_recursive_print(root->left, pFile, fun);
	}
	fun(root->data , pFile);
	if (root->right != NULL) {
		avltree_for_each_recursive_print(root->right, pFile, fun);
	}
}
void avltree_for_each_print(const avltree * tree, FILE * pFile, avltree_for_print_func fun)
{
	if (tree->root) {
		avltree_for_each_recursive_print(tree->root,pFile, fun);
	}
}

static void avltree_for_each_recursive_print_containers(const avltreenode * root, FILE *pFile, void* data_set, avltree_for_print_conainers_func fun)
{
	if (root->left != NULL) {
		avltree_for_each_recursive_print_containers(root->left, pFile, data_set, fun);
	}
	fun(root->data, pFile, data_set);
	if (root->right != NULL) {
		avltree_for_each_recursive_print_containers(root->right, pFile, data_set, fun);
	}
}
...
{
	if (tree->root) {
		avltree_for_each_recursive_print_containers(tree->root, pFile, data_set, fun);
	}
}
...
static avltreenode * avltreenode_create(void * data, PMemory_pool pool)
{
	Dedup_Error_Val ret_val = SUCCESS;
	avltreenode *node = NULL;
	ret_val = memory_pool_alloc(pool, sizeof(avltreenode), (uint32**)&node);
	assert(ret_val == SUCCESS);
...
void* avltree_add(avltree * tree, void * data, PMemory_pool pool)
{
    ...
    if (avltree_search(tree, &result, data)) {
        ...
        avltreenode *node = avltreenode_create(data, pool);
